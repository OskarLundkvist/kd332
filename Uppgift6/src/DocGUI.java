...
public class DocGUI extends JFrame {
	...
	public DocGUI() {
		...
		brph = new BarGraph();
		brph.setBounds(10, 92, 414, 343);
		contentPane.add(brph);
		repaint();
		...
		Program program0 = (Program) comboBox.getItemAt(0);
    	brph.updateValues(program0.getWomen(), program0.getMen());
		comboBox.setBounds(10, 60, 414, 20);
		comboBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
            	Program program = (Program) comboBox.getSelectedItem();
            	brph.updateValues(program.getWomen(), program.getMen());
        		repaint();
            }
        });
